...
export const WalletUpdate = (props: IWalletUpdateProps) => {
  ...
  const { walletEntity, users, loading, updating } = props;
  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="parkingSystemApplicationApp.wallet.home.createOrEditLabel">Create or edit a Wallet</h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
        <Col md="8">
          {loading ? (
            ...
            <AvForm model={isNew ? {} : walletEntity} onSubmit={saveEntity}>
              {!isNew ? (
                <AvGroup>
                  <Label for="wallet-id">ID</Label>
                  <AvInput id="wallet-id" type="text" className="form-control" name="id" required readOnly />
                </AvGroup>
              ) : null}
              <AvGroup>
                <Label id="moneyLabel" for="wallet-money">
                  Money
                </Label>
                <AvField id="wallet-money" type="string" className="form-control" name="money" />
              </AvGroup>
              <AvGroup>
                <Label for="wallet-user">User</Label>
                <AvInput
                  id="wallet-user"
                  type="select"
                  className="form-control"
                  name="user.id"
                  value={isNew ? users[0] && users[0].id : walletEntity.user?.id}
                  required
                >
                  {users
